...
class GadgetVectorizer:
    ...
    """
    Takes a line of Java code (string) as input
    Tokenizes code (breaks down into identifier, variables, keywords, operators)
    ...
    Number of tokens used is min of number_of_tokens and 50 (or as set)

    """
    def vectorize(self, gadget):
        tokenized_gadget, backwards_slice = GadgetVectorizer.tokenize_gadget(gadget)
        vectors = numpy.zeros(shape=(100, self.vector_length))
        if backwards_slice:
            for i in range(min(len(tokenized_gadget), 50)):
                vectors[100 - 1 - i] = self.embeddings[tokenized_gadget[len(tokenized_gadget) - 1 - i]]
        else:
            for i in range(min(len(tokenized_gadget), 50)):
